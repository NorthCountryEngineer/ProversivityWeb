...
import { updateMeeting, updateRelationship } from "../graphql/mutations";
function ArrayField({
  items = [],
  onChange,
  label,
...
export default function RelationshipUpdateForm(props) {
  ...
  const resetStateValues = () => {
    ...
    setRequestor(cleanValues.requestor);
    setCurrentRequestorValue(undefined);
    setCurrentRequestorDisplayValue("");
    setEmployee(cleanValues.employee);
    ...
    setUserRelationshipsId(cleanValues.userRelationshipsId);
  ...
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ...
        : relationshipModelProp;
      const requestorRecord = record ? await record.requestor : undefined;
      setRequestor(requestorRecord);
      const employeeRecord = record ? await record.employee : undefined;
      setEmployee(employeeRecord);
      const linkedMeetings = record?.meetings?.items ?? [];
      setLinkedMeetings(linkedMeetings);
      const userRelationshipsIdRecord = record
        ? record.userRelationshipsId
        : undefined;
      const userRecord = userRelationshipsIdRecord
        ? (
            await API.graphql({
              query: getUser.replaceAll("__typename", ""),
              variables: { id: userRelationshipsIdRecord },
            })
          )?.data?.getUser
        : undefined;
      setUserRelationshipsId(userRelationshipsIdRecord);
      setSelectedUserRelationshipsIdRecords([userRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          userRelationshipsId: userRelationshipsId ?? null,
        };
        ...
        try {
          ...
          meetingsToUnLink.forEach((original) => {
            if (!canUnlinkMeetings) {
              throw Error(
                `Meeting ${original.id} cannot be unlinked from Relationship because relationshipMeetingsId is a required field.`
              );
            }
            promises.push(
              API.graphql({
            ...
            promises.push(
              API.graphql({
                ...
                variables: {
                  input: {
                    id: original.id,
                    relationshipMeetingsId: relationshipRecord.id,
                  },
                },
              })
            );
          ...
          const modelFieldsToSave = {
            ...
            relationshipRequestorId: modelFields?.requestor?.id ?? null,
            relationshipEmployeeId: modelFields?.employee?.id ?? null,
            userRelationshipsId: modelFields.userRelationshipsId ?? null,
          };
          promises.push(
            API.graphql({
    ...
    >
      ...
      >
        <Autocomplete
          ...
          defaultValue={userRelationshipsId}
          onChange={(e) => {
            let { value } = e.target;
