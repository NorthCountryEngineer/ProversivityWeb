...
export default function BlogPageCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const blogPage = await DataStore.save(
            new BlogPage(modelFieldsToSave)
          );
          const promises = [];
              ...
              promises.push(
                DataStore.save(
                  Blog.copyOf(original, (updated) => {
                    updated.blogPagePostsId = blogPage.id;
                    updated.blogPage = blogPage;
                  })
                )
              );
      ...
      {...getOverrideProps(overrides, "BlogPageCreateForm")}
      {...rest}
    >
      <TextField
        label="Title"
