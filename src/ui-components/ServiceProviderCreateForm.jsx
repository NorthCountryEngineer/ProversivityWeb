...
export default function ServiceProviderCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const serviceProvider = await DataStore.save(
            new ServiceProvider(modelFields)
          );
          const promises = [];
          ...
          if (userToLink) {
            promises.push(
              DataStore.save(
                User.copyOf(userToLink, (updated) => {
                  updated.serviceProviderProfile = serviceProvider;
                })
              )
            );
            const serviceProviderToUnlink =
              await userToLink.serviceProviderProfile;
            if (serviceProviderToUnlink) {
              if (
                ...
                throw Error(
                  `User ${userToLink.id} cannot be linked to ServiceProvider because it is already linked to another ServiceProvider.`
                );
              }
            }
          }
      ...
      {...getOverrideProps(overrides, "ServiceProviderCreateForm")}
      {...rest}
    >
      <ArrayField
        lengthLimit={1}
